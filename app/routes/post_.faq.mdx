...
import MDXTemplate from "@/layouts/MDXLayout"
...
export const meta = () => {
  return [{title: metaObject.title}, {name: "description", content: metaObject.description}];
}
...
export default ({ children }) => <MDXTemplate meta={metaObject}>{children}</MDXTemplate>
